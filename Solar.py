...
def Auswertung_Dataframe(Dataframe_in):
    df_Jahressummen=Dataframe_in.groupby('Year').agg({'P':'sum','Verbrauch':'sum', 'Netzbezug':'sum', 'NetzEinspeisung':'sum', 'Direkte_PV_Nutzung':'sum', 'Akkunutzung':'sum'}).round(0)
    #Jahresarbeitszahl-Auswertung
    df_Jahressummen=Dataframe_in.groupby('Year').agg({'P':'sum', 'T2m':'mean','Heizleistung':'sum','Warmwasserleistung':'sum','Stromaufnahme_WP_Heiz':'sum','Stromaufnahme_WP_Warmwasser':'sum','Stromaufnahme_Haus':'sum'}).round(2)
